...
// --- Mercury Model (rotation + revolution + color) ---
function MercuryModel({ onHoverChange, onClick }: {onHoverChange?: (b: boolean)=>void, onClick?: ()=>void}) {
    ...
    scene.traverse((child: any) => {
      if (child.isMesh) {
        ...
        materials.forEach((m: any) => {
          ...
          m.transparent = false;
          m.opacity = 1;
          m.color = new THREE.Color("#A8A8A8");
          m.metalness = 0.45;
          m.roughness = 0.7;
        });
      }
...
function Sidebar({ isOpen, closeSidebar }: { isOpen: boolean, closeSidebar: () => void }) {
  ...
  return (
    <div className={`fixed top-0 right-0 h-full w-64 z-[300] transform transition-transform duration-500 bg-black/90 ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
      <button
        onClick={closeSidebar}
        className="absolute top-4 right-4 text-white text-xl font-bold z-[310]"
      >✕</button>
      <ul className="flex flex-col mt-32 ml-6 space-y-8 text-white text-2xl">
        <li
          className="cursor-pointer hover:text-pink-400"
          onClick={() => { closeSidebar(); router.push("/about"); }}
        >ABOUT</li>
        <li
          className="cursor-pointer hover:text-pink-400"
          onClick={() => { closeSidebar(); router.push("/contact"); }}
        >CONTACT</li>
        <li
          className="cursor-pointer hover:text-pink-400"
          onClick={() => { closeSidebar(); router.push("/details"); }}
        >PROFILE</li>
      </ul>
    </div>
  );
...
export default function MercuryPage() {
  ...
  const mercuryX = useTransform(scrollYProgress, [0.26, 0.36], ["0vw", "18vw"]);
  const mercuryY = useTransform(scrollYProgress, [0, 1], ["8vh", "12vh"]);
  const mercuryScale = useTransform(scrollYProgress, [0.26, 0.36], [1, 1.3]);
  ...
  return (
    <div className="w-full min-h-[240vh] bg-black relative cursor-none overflow-x-hidden">
      ...
      <motion.div style={{ opacity: heroOpacity, y: heroY }} className="fixed inset-0 z-30">
        <div className="absolute inset-0 flex items-center justify-center" style={{ top: "8vh" }}>
          <span className="font-extrabold uppercase leading-none"
            style={{
              ...
              color: "#C0C0C0",
              textShadow: "0 12px 80px #02051e33, 0 3px 18px #303a46cc, 0 1px 0 #fff8",
        ...
        <AnimatePresence>
          {showTypewriter && (
            ...
            >
              <div className="relative bg-gray-50/90 text-gray-800 px-7 py-3 rounded-2xl shadow-xl border border-gray-300 font-semibold text-lg md:text-xl text-center">
                <TypewriterText text="☿ Click on Mercury to explore your curious mind! ☿" />
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {/* Mercury 3D Model Foreground */}
      <motion.div
        className="fixed z-[120] flex items-center justify-center pointer-events-none"
        style={{
          top: 0,
          left: 0,
          width: "100vw",
      ...
      >
        <div
          ...
          style={{
            ...
            height: "clamp(560px,50vw,1100px)",
          }}
        >
          <Canvas
          ...
          >
            ...
            <directionalLight position={[10, 10, 10]} intensity={2.9} castShadow />
            <Suspense
              fallback={
                <mesh>
            ...
            <OrbitControls enableZoom={false} enablePan={false} enabled={!sidebarOpen} />
          </Canvas>
        </div>
      </motion.div>
      ...
      <motion.div style={{ opacity: cardOpacity, y: cardY, marginLeft: "clamp(2vw,8vw,16vw)" }}
        ...
        bg-gradient-to-br from-gray-200/30 via-black/85 to-gray-900/40
        border border-gray-400/40 shadow-[0_0_40px_rgba(192,192,192,0.25)]
        backdrop-blur-xl"
      >
        <motion.h2 className="text-3xl md:text-4xl font-extrabold text-center mb-4"
          ...
          style={{
            background: "linear-gradient(90deg,#ececec 40%,#bababa 100%)",
            WebkitBackgroundClip: "text",
            WebkitTextFillColor: "transparent",
          }}
        >
          ⚡ Numerology No. 5 – Mercury
        </motion.h2>
        <p className="text-base md:text-lg text-gray-100/90 font-medium text-center">
          Symbol of <span className="font-semibold text-gray-200">curiosity, adaptability, and intellect</span>.
          Mercury inspires creative thinking, exploration, and versatile communication.
        </p>
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          {["Curiosity", "Versatility", "Adaptability", "Intellect", "Communication"].map((trait, i) => (
            <span key={i}
              className="px-3 py-1 rounded-full bg-gray-900/40 border border-gray-400/30 
              shadow-[0_0_12px_rgba(180,180,180,0.18)] text-gray-100 text-sm md:text-base font-semibold"
            >
              {trait}
            </span>
          ))}
