...
function Sidebar({ isOpen, closeSidebar }: { isOpen: boolean; closeSidebar: () => void }) {
  ...
  return (
    <div
      className={`fixed top-0 right-0 h-full w-64 z-[300] transform transition-transform duration-500 bg-black/90 ${isOpen ? "translate-x-0" : "translate-x-full"
        }`}
    >
      <button
...
export default function NeptunePage() {
  ...
  const neptuneX = useTransform(scrollYProgress, [0.26, 0.36], ["0vw", "18vw"]);
  const neptuneScale = useTransform(scrollYProgress, [0.26, 0.36], [1, 1.3]);
    ...
    <div className="w-full min-h-[240vh] bg-black relative cursor-none overflow-x-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center" style={{ top: "8vh" }}>
          <span className="font-extrabold uppercase leading-none"
            style={{
              ...
              color: "#60A5FA",
              textShadow: "0 12px 80px #001a, 0 3px 18px #003a, 0 1px 0 #fff5",
          ...
          {showTypewriter && (
            ...
            >
              <div className="relative bg-blue-50/90 text-blue-800 px-7 py-3 rounded-2xl shadow-xl border border-blue-300 font-semibold text-lg md:text-xl text-center">
                <TypewriterText text="🌊 Click on Neptune to explore intuition, dreams, and manifestation! 🌊" />
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      >
        <motion.h2 className="text-3xl md:text-4xl font-extrabold text-center mb-4"
          ...
          style={{
            background: "linear-gradient(90deg,#dbeafe 40%,#60a5fa 100%)",
            WebkitBackgroundClip: "text",
            WebkitTextFillColor: "transparent",
          }}
        >
          🌊 Numerology No. 4 – Neptune
        </motion.h2>
        <p className="text-base md:text-lg text-blue-100/90 font-medium text-center">
          Symbol of <span className="font-semibold text-blue-200">intuition, manifestation, and dreams</span>.
          They blend creativity with discipline, guiding visions into reality.
        </p>
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          {["Intuition", "Dreams", "Imagination", "Compassion", "Vision"].map((trait, i) => (
            <span key={i}
              className="px-3 py-1 rounded-full bg-blue-900/40 border border-blue-400/30 
              shadow-[0_0_12px_rgba(100,180,255,0.35)] text-blue-100 text-sm md:text-base font-semibold"
            >
              {trait}
            </span>
          ))}
