...
// --- Mars 3D Model (static, spinning, clickable) ---
function MarsModel({ onHoverChange, onClick }: { onHoverChange?: (v: boolean) => void, onClick?: () => void }) {
  const { scene } = useGLTF("/models/mars/scene.gltf");
...
const TypewriterText = ({ text }: { text: string }) => {
  ...
  return <span>{visible}<span className="text-red-400 animate-pulse">|</span></span>;
};
...
function Sidebar({ isOpen, closeSidebar }: { isOpen: boolean; closeSidebar: () => void }) {
  ...
  return (
    <div className={`fixed top-0 right-0 h-full w-64 z-[120] transition-all duration-500 bg-black/90 ${isOpen ? "translate-x-0 opacity-100 pointer-events-auto" : "translate-x-full opacity-0 pointer-events-none"}`}>
...
export default function MarsPage() {
  ...
  const marsX = useTransform(scrollYProgress, [0.26, 0.36], ["0vw", "18vw"]);
  const marsScale = useTransform(scrollYProgress, [0.26, 0.36], [1, 1.18]);
    ...
    <div className="w-full min-h-[240vh] bg-black relative cursor-none overflow-x-hidden">
      ...
      <motion.div style={{ opacity: heroOpacity, y: heroY, pointerEvents: heroOpacity ? "auto" : "none" }} className="fixed inset-0 z-30">
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none" style={{ top: "8vh", position: "absolute", zIndex: 2 }}>
          <span
            ...
            style={{
              ...
              color: "#ff6b57",
              textShadow: "0 12px 80px #000a, 0 3px 18px #af2a00a6, 0 1px 0 #fff7",
        ...
        <AnimatePresence>
          {showTypewriter && (
            ...
            >
              <div className="relative bg-red-100/90 text-red-900 px-7 py-3 rounded-2xl shadow-xl border border-red-400 font-semibold text-lg md:text-xl text-center min-w-[300px] max-w-[480px]">
                <TypewriterText text="🔥 Click on Mars to awaken the warrior’s spirit! 🔥" />
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      >
        ...
        >
          ...
          >
            ...
            <Suspense fallback={<mesh><sphereGeometry args={[2,32,32]} /><meshStandardMaterial color="#ff6b57" /></mesh>}>
              {/* ✅ Enable clicks only on Mars */}
              <group>
                <MarsModel
      ...
      <motion.div
        ...
        className={`
          ...
          bg-gradient-to-br from-red-400/40 via-black/85 to-red-900/60
          border border-red-300/40 shadow-[0_0_36px_rgba(255,120,80,0.18)]
          backdrop-blur-xl
        `}
        initial={{ opacity: 0 }}
      >
        <motion.h2
          ...
          style={{
            background: "linear-gradient(90deg,#fee2e2 40%,#fd725c 100%)",
            WebkitBackgroundClip: "text",
            WebkitTextFillColor: "transparent",
          }}
        >
          🔥 Numerology No. 9 – Mars
        </motion.h2>
        <p className="text-base md:text-lg text-red-100/90 font-medium text-center">
          Symbol of <span className="font-semibold text-red-200">courage, passion, and resilience</span>.<br />
          Mars guides assertive leaders and empowers the spirit with strength and perseverance.
        </p>
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          {["Strength", "Courage", "Passion", "Wisdom", "Resilience"].map((trait, i) => (
            <span
              key={i}
              className="px-3 py-1 rounded-full bg-red-900/40 border border-red-400/30 
                shadow-[0_0_12px_rgba(250,100,40,0.25)] text-red-100 text-sm md:text-base font-semibold"
            >
              {trait}
            </span>
          ))}
