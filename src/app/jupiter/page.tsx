...
function GalaxyBackground() {
  ...
  // Starfield and nebula positions/colors
...
const TypewriterText = ({ text }: { text: string }) => {
  ...
  return (<span>{visible}<span className="text-yellow-300 animate-pulse">|</span></span>);
};
...
function Sidebar({ isOpen, closeSidebar }: { isOpen: boolean; closeSidebar: () => void }) {
  ...
  return (
    <div
      className={`fixed top-0 right-0 h-full w-64 z-[200] transform transition-transform duration-500 bg-black/90 ${
...
export default function JupiterPage() {
  ...
  const jupiterX = useTransform(scrollYProgress, [0.26, 0.36], ["0vw", "18vw"]);
  const jupiterScale = useTransform(scrollYProgress, [0.26, 0.36], [1, 1.3]);
    ...
    <div className="w-full min-h-[240vh] bg-black relative cursor-none overflow-x-hidden">
      ...
      <motion.div style={{ opacity: heroOpacity, y: heroY }} className="fixed inset-0 z-30">
        ...
        <AnimatePresence>
          {showTypewriter && (
            ...
            >
              <div className="relative bg-yellow-50/90 text-yellow-800 px-7 py-3 rounded-2xl shadow-xl border border-yellow-300 font-semibold text-lg md:text-xl text-center">
                <TypewriterText text="🪐 Click on Jupiter to expand your wisdom and opportunities! 🪐" />
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      >
        <div
          ...
          style={{ width: "clamp(320px,30vw,560px)", height: "clamp(320px,30vw,560px)" }}
        >
          <Canvas camera={{ position: [0, 0, 16], fov: 56 }} style={{ background: "none", width: "100%", height: "100%" }}>
            ...
            >
              <JupiterModel onHoverChange={setShowTypewriter} onClick={() => router.push("/game")} />
            </Suspense>
      ...
      <motion.div style={{ opacity: cardOpacity, y: cardY, marginLeft: "clamp(2vw,8vw,16vw)" }}
        ...
        bg-gradient-to-br from-yellow-200/30 via-black/85 to-yellow-900/40
        border border-yellow-400/40 shadow-[0_0_40px_rgba(255,223,100,0.25)]
        backdrop-blur-xl"
      >
        ...
        >
          🪐 Numerology No. 3 – Jupiter
        </motion.h2>
        <p className="text-base md:text-lg text-yellow-100/90 font-medium text-center">
          Symbol of <span className="font-semibold text-yellow-200">growth, wisdom, and optimism</span>.
          Bringers of expansion and joy, they inspire confidence and embrace life’s opportunities.
        </p>
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          {["Wisdom", "Growth", "Optimism", "Expansion", "Joy"].map((trait, i) => (
            <span key={i}
              className="px-3 py-1 rounded-full bg-yellow-900/40 border border-yellow-400/30 
              shadow-[0_0_12px_rgba(250,200,50,0.35)] text-yellow-100 text-sm md:text-base font-semibold"
            >
              {trait}
            </span>
          ))}
